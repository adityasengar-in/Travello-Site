  ...
  return (
    <nav className="fixed left-1/2 top-4 z-40 w-[calc(100%-2rem)] max-w-[980px] -translate-x-1/2 rounded-full backdrop-blur-xl">
      <div className="mx-auto flex min-h-16 items-center justify-between px-4 py-2 sm:px-6">
        <a href="/" className="flex items-center gap-2 text-3xl font-bold tracking-normal text-[#101913]">
          Travello
        </a>
        ...
        <div className="hidden items-center rounded-full border border-black/10 bg-white p-1 text-sm font-semibold text-[#2d3a31] shadow-sm md:flex">
          {navItems.map((item) => (
            <a key={item.href} className="nav-pill px-4 py-2" href={item.href}>
              {item.label}
            </a>
          ))}
        </div>
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          <a className="inline-flex min-w-28 items-center justify-center rounded-full bg-[#132318] px-5 py-2.5 text-sm font-bold text-white transition hover:bg-[#02ae52]" href="#plan">
            Plan a trip
          </a>
        </div>
        ...
        <details className="mobile-nav relative md:hidden">
          ...
          <div className="absolute right-0 mt-3 flex min-w-48 flex-col gap-1 rounded-lg border border-black/10 bg-white p-2 text-sm font-semibold shadow-xl">
            {navItems.map((item) => (
              <a key={item.href} className="rounded-md px-4 py-3 text-[#101913] hover:bg-[#f2f7ef]" href={item.href}>
                {item.label}
              </a>
            ))}
            <a className="rounded-md px-4 py-3 text-[#101913] hover:bg-[#f2f7ef]" href="#signin">
              Sign in
            </a>
          </div>
        </details>
